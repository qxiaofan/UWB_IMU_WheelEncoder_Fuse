...
void LoadUWBMeasure(std::string file_path, std::vector<uwb_observe> &all_observation)
{
    ...
    if (!file.is_open())
    {
        likuncout << "open file failed !" << std::endl;
        return;
    }
...
//        likuncout << split_data[0] << "; " << split_data[1] << "; " << split_data[2] << "; " << split_data[3] << std::endl;
    ...
    likuncout << "all_uwb.size() = " << all_observation.size() << std::endl
...
void LoadIMUMeasure(std::string file_path, std::vector<imu_observe> &all_observation)
{
    ...
    while(!file.eof())
    {
        ...
        if(split_data.size() != 10)
            likuncout << "error in load imu data" << std::endl;
    ...
    likuncout << "all_imu.size() = " << all_observation.size() << std::endl
              << "data 0 = " << all_observation[0].timestamp
...
void LoadWheelMeasure(std::string file_path, std::vector<wheel_observe> &all_observation)
{
    ...
    likuncout << "all_wheel.size() = " << all_observation.size() << std::endl
              << "data 0 = " << all_observation[0].timestamp
...
void LoadVslamMeasure(std::string file_path, std::vector<vslam_observe> &all_observation)
{
    ...
    while(!file.eof())
    {
        ...
        vslam_oi.vslam = cv::Point3d(split_data[11], split_data[12], split_data[13]);
    ...
    likuncout << "all_vslam.size() = " << all_observation.size() << std::endl
              << "data 0 = " << all_observation[0].timestamp
