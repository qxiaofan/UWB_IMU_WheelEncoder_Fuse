...
size_t FindNearstImu(std::vector<Eigen::Vector2d> &imu_yaw_normal, size_t &start, double &est_time)
{
    ...
    likuncout << "error in find nearst imu timestamp for vslam" << std::endl;
...
void RunTimeCalibration(std::vector<imu_observe> &imu_observe, std::vector<wheel_observe> &wheel_observe, double &time_imu_vslam)
{
    ...
    likuncout << vslam_window.size() << std::endl;
    if(vslam_window.size() < 20)
        likuncout << "no rotation in vslam data, can't calibration timestamp" << std::endl;
    ...
    for(size_t i = 0; i < imu_yaw_normal.size(); i++)
    {
        ...
        if(push)
        {
//            likuncout << "imu i: " << i << "; error_tmp: " << error_tmp << std::endl;
    ...
    if(imu_final == -1)
        likuncout << "error in time calibration, please check" << std::endl;

    time_imu_vslam = imu_yaw_normal[size_t(imu_final)][0] - vslam_window[0][0];

    likuncout << "time_imu_vslam: " << time_imu_vslam << std::endl;
                        ...
                        Eigen::Matrix4d &pose_uwb_vslam)
{
    ...
    likuncout << "size of vslam and uwb matches: " << matches.size() << std::endl;
    if(matches.size() < 20)
        return;
    ...
    likuncout <<  "translate: " << translate.transpose() << "; eular angle: " << eula_angle.transpose() << std::endl << std::endl;
